...
def test_initial_conditions():
    ...
    assert np.isclose(
        sol.y[0][0], theta0
    ), f"Expected theta0 = {theta0}, got {sol.y[0][0]}"
...
def test_zero_length():
    ...
    with pytest.raises(
        ZeroDivisionError
    ):  # Division par zéro dans l'équation du mouvement
...
def test_large_angle():
    ...
    assert np.all(
        np.abs(theta_normalized) < 100 * np.pi
    ), f"Angle exceeds expected range, got {theta_normalized[0]}"
...
def test_energy_conservation():
    ...
    assert (
        energy_diff < 1e-1
    ), f"Energy conservation violated, max difference {energy_diff}"
